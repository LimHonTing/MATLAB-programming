...
rng('default');
%seed 507, sel= 0.4 mul = 0.4
%seed 960, sel= 0.4 mul = 0.7
%seed 1068, sel= 0.2 mul = 0.4
rng(507);

% load dataset
data = readmatrix("fertility_Diagnosis.csv");
...
% hidden = randi([1,max_hidden]);
% hidden_units_ea = randi([1,max_hidden_units_ea], 1, hidden);
...
selrate = 0.4; % SelectionRate
mutrate = 0.4; % MutationRate
progress = [];
...
disp(hidden_layer_mean);
disp(hidden_layer_std);
...
[a, b, c] = splitpopulation(population, num_genes);

% calculate mean of hidden layers
hidden_layer_mean = mean(b);
% calculate std of hidden layers
hidden_layer_std = std(b);
...
% calculate mean of hidden layer units
hidden_layer_unit_mean = nanmean(c, 1);
% calculate std of hidden layer units
hidden_layer_unit_std = nanstd(c, 1);
